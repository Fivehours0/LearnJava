 ...
 * 实现解耦， 使得两者可以独立地变化。 ）
 *
 * 就像书中的例子，product是耦合在company内部的，如果有一个类继承了company类，由于product容易有修改，会导致子类的不稳定。所以可以把product从
 * company中抽离出去
 *
 * ● Abstraction——抽象化角色
 * 它的主要职责是定义出该角色的行为， 同时保存一个对实现化角色的引用， 该角色一般
 * 是抽象类。在这里，抽象化角色指的是笔，笔内有一个颜色的引用
 * ● Implementor——实现化角色
 * 它是接口或者抽象类， 定义角色必需的行为和属性。行为和属性，在这里指的是笔的颜色
